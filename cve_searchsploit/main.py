...
def iter_edbid_from_cve(cve):
    ...
    with open(pdir + "/exploit-database/files_exploits.csv") as files:
        ...
        for row in reader:
            edb, file, description, date, author, platform, type, port = tuple(
                row)
        ...
        for i in range(csv_len):
            ...
            if edb in data:
                ...
                while True:
                    try:
                        r = requests.get(
                            "https://www.exploit-db.com/exploits/" + edb, headers=get_header)
                        content = r.text
                    except Exception:
                        time.sleep(10)
                        continue
...
if not os.path.isdir(pdir + "/exploit-database"):
    ...
    os.system(
        "cd %s; git clone https://github.com/offensive-security/exploit-database" % pdir)
